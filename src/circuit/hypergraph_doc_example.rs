...
mod tests {
    ...
    fn test_doc_main() -> std::result::Result<(), Box<dyn std::error::Error>> {
        ...
        let _fourth_relation = graph.add_hyperedge(vec![daena], playing_online)?;
        let fifth_relation =
            graph.add_hyperedge(vec![ewan, charles, bianca, bianca, ewan], passing_ball)?;
        ...
        assert_eq!(
            ...
            Ok(vec![first_relation, second_relation, third_relation])
        );
        assert_eq!(
            graph.get_full_vertex_hyperedges(VertexIndex(0)),
            Ok(vec![
                vec![faarooq, ava, ghanda],
                vec![faarooq, ava, ghanda],
                vec![ewan, ava, bianca]
            ])
        );
        ...
        assert_eq!(
            graph.get_hyperedges_intersections(vec![second_relation, third_relation]),
            Ok(vec![ava])
        );

        // Find a hyperedge containing a connection between two vertices.
        assert_eq!(
            graph.get_hyperedges_connecting(bianca, bianca),
            Ok(vec![fifth_relation])
        );

        // Get the adjacent vertices from a vertex.
        assert_eq!(
            graph.get_adjacent_vertices_from(VertexIndex(0)),
            Ok(vec![bianca, ghanda])
        );
        ...
        assert_eq!(
            graph.get_dijkstra_connections(faarooq, bianca),
            Ok(vec![
                (faarooq, None),
                (ava, Some(first_relation)),
                (bianca, Some(third_relation))
            ])
        );
        ...
        graph.update_hyperedge_weight(
            third_relation,
            Relation::new("share a viral video with a capybara", 1),
        )?;
        ...
        let _ = graph.join_hyperedges(&[fifth_relation, third_relation]);
