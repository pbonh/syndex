...
impl Resistor {
    fn resistor_node_subst(p_node: &CircuitNode, n_node: &CircuitNode) -> DeviceEquation {
    ...
    pub(crate) fn resistor_variable_ctx(
        p_node: &CircuitNode,
        n_node: &CircuitNode,
    ) -> Vec<VariableContext> {
        let vpn = CircuitNode::from_str("vpn").expect("Invalid CircuitNode Name.");
        let vpn_eq = Self::resistor_node_subst(&p_node, &n_node);
        vec![(vpn, vpn_eq)]
    }
}
